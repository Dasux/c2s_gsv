...
\noindent For this workstation deployment, we are using \textbf{Red Hat Enterprise Linux 7.9.0}. This version is
selected because many EDA tools and supporting libraries are validated against older enterprise
Linux releases, and RHEL 7.9.0 provides a mature, well-documented platform with broad community
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\textwidth]{resources/rhel7.9.0.png}
\end{figure}
...
Click \textbf{"Install Red Hat Enterprise Linux 7.9"}, and follow the setup prompts. For the "Installation Destination" on the Virtual Machine, it should automatically select the Virtual Disk Space (.vhdx) you set while setting up the Virtual Machine.
...
\section{Native Dual Booting RHEL 7.9.0}
...
This is the main step for dual booting, and usually the place where most novices mess up. Most of the steps are the same as the virtualization setup. The only thing we're going to have to do differently here is selecting the disk where the base system will be installed. Click on the \textbf{"Installation Destination"} option from the image below, and select the disk we chose in Subsection~\ref{subsec:disk} -- \textbf{\textit{nvme1n1}}
	...
	\begin{figure}[H]
		\centering
		\fbox{\parbox[c][5cm][c]{0.5\textwidth}{\centering Installation destination screenshot pending}}
...
\begin{tcolorbox}
	\noindent This chapter established the installation baseline for deploying \textbf{RHEL 7.9.0} on a workstation intended for Cadence and other EDA workflows. We first compared the two supported deployment models: virtualization and native dual booting. Virtualization is useful for testing, training, and low-risk experimentation, while native dual booting is the preferred approach for production-style EDA work because it gives RHEL direct access to the workstation hardware, storage, memory, graphics stack, network interface, and any license-related devices.

	\vspace{0.2cm}
	\noindent We then identified the hardware expectations for a practical EDA workstation, including sufficient RAM, SSD/NVMe storage, CPU capacity, stable graphics support, and reliable network connectivity. The specific HP workstation used in this deployment was documented so that later installation decisions could be tied back to the actual machine configuration. The chapter also covered how to obtain the correct RHEL installation ISO, why the full DVD installer is preferred over the boot ISO, and why RHEL 7.9.0 remains suitable for toolchains that depend on older enterprise Linux compatibility.

	\vspace{0.2cm}
	\noindent The firmware layer was then reviewed through the distinction between legacy BIOS, UEFI, and Secure Boot. For modern dual-boot workstations, the important rule is to keep the Windows and RHEL installations in the same boot mode. In our case, the system uses \textbf{UEFI}, so the RHEL installation media and final installation should also follow the UEFI path. This avoids mixed bootloader behavior and makes the system easier to recover and maintain.

	\vspace{0.2cm}
	\noindent For the virtualized setup, the chapter showed how to prepare a RHEL virtual machine using Hyper-V, attach the downloaded ISO, configure the virtual firmware and DVD boot order, disable Secure Boot inside the VM, and complete the base installation. This path provides a convenient sandbox for learning the installer and validating commands before making changes to the physical machine.

	\vspace{0.2cm}
	\noindent For native dual booting, the chapter introduced the partitioning strategy required for a Linux workstation, including the EFI partition, root partition, and swap partition. It also explained how Windows disk numbering maps to Linux NVMe device names such as \texttt{nvme0n1} and \texttt{nvme1n1}. This mapping is critical because selecting the wrong disk during installation can overwrite the existing operating system or user data.

	\vspace{0.2cm}
	\noindent Finally, we created the bootable RHEL installation USB using Rufus, selected the correct ISO, and chose settings appropriate for the target workstation: \textbf{GPT} partition scheme, \textbf{UEFI} target system, and the default filesystem and cluster-size options unless Rufus requires otherwise. At the end of the chapter, the machine is ready to boot from the USB installer and proceed to the RHEL installation destination stage, where the correct target disk must be selected carefully before installation begins.
\end{tcolorbox}
